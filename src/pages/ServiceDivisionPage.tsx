import { Link, Navigate, useParams } from "react-router-dom";
import { ArrowLeft, ArrowUpRight, Check } from "lucide-react";
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import SiteSEO from "@/components/site/SiteSEO";
import LeadCtaRow from "@/components/site/LeadCtaRow";
import EnquiryForm from "@/components/site/EnquiryForm";
import StickyContactBar from "@/components/site/StickyContactBar";
import { getDivision } from "@/content/divisions";
import { buildServiceSchema } from "@/lib/seoSchema";
import { BRAND } from "@/lib/brand";

const ServiceDivisionPage = () => {
  const { slug = "" } = useParams();
  const division = getDivision(slug);
  if (!division) return <Navigate to="/services" replace />;

  const path = `/services/${division.slug}`;
  const seoDescription = `${division.intro} Delivered across Johannesburg, Sandton, Durban and KwaZulu-Natal by ${BRAND.name}.`;

  return (
    <div className="min-h-screen bg-background">
      <SiteSEO
        title={`${division.title} — Johannesburg & Durban | ${BRAND.name}`}
        description={seoDescription.slice(0, 300)}
        path={path}
        jsonLd={buildServiceSchema({
          serviceType: division.title,
          description: division.intro,
          path,
        })}
      />
      <Header />

      <section className="bg-background border-b border-foreground/10">
        <div className="container mx-auto px-6 lg:px-10 py-20 md:py-28">
          <div className="max-w-4xl">
            <Link
              to="/services"
              className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.24em] text-foreground/60 hover:text-foreground mb-8"
            >
              <ArrowLeft className="h-3.5 w-3.5" /> All services
            </Link>
            <p className="text-[11px] uppercase tracking-[0.28em] text-foreground/60 mb-5">
              {division.label} · Johannesburg &amp; Durban
            </p>
            <h1 className="font-display font-light text-4xl md:text-6xl tracking-[-0.02em] text-foreground leading-[1.04]">
              {division.title}
            </h1>
            <p className="mt-8 text-lg md:text-xl text-foreground/75 leading-relaxed max-w-2xl">
              {division.intro}
            </p>
            <div className="mt-10">
              <LeadCtaRow
                service={division.leadService}
                primaryLabel={division.ctaLabel}
                whatsappMessage={`Hi Siyakha, I'd like help with ${division.title.toLowerCase()}.`}
                context={`${division.slug} hero`}
              />
            </div>
            <p className="mt-5 text-[13px] text-foreground/60">{BRAND.ownerReplyLine}</p>
          </div>
        </div>
      </section>

      <section className="bg-background border-b border-foreground/10">
        <div className="container mx-auto px-6 lg:px-10 py-16 md:py-24">
          <div className="grid md:grid-cols-2 gap-12 md:gap-20">
            <div>
              <h2 className="font-display font-light text-2xl md:text-3xl tracking-tight text-foreground mb-6">
                What we deliver
              </h2>
              <ul className="space-y-3">
                {division.offers.map((o) => (
                  <li key={o} className="flex gap-3 text-sm text-foreground/75 leading-relaxed">
                    <Check className="h-4 w-4 mt-0.5 flex-shrink-0 text-foreground" strokeWidth={1.5} />
                    {o}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h2 className="font-display font-light text-2xl md:text-3xl tracking-tight text-foreground mb-6">
                What changes for you
              </h2>
              <ul className="space-y-4">
                {division.outcomes.map((o) => (
                  <li
                    key={o}
                    className="border-l border-foreground/20 pl-4 text-sm text-foreground/75 leading-relaxed"
                  >
                    {o}
                  </li>
                ))}
              </ul>
              {division.capabilityNote && (
                <p className="mt-8 border border-foreground/15 p-5 text-[13px] text-foreground/70 leading-relaxed">
                  {division.capabilityNote}
                </p>
              )}
              {division.related?.length ? (
                <div className="mt-8 space-y-2">
                  {division.related.map((r) => (
                    <Link
                      key={r.to}
                      to={r.to}
                      className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-foreground/70 hover:text-foreground"
                    >
                      {r.label} <ArrowUpRight className="h-3.5 w-3.5" />
                    </Link>
                  ))}
                </div>
              ) : null}
            </div>
          </div>
        </div>
      </section>

      <section id="enquiry" className="bg-foreground text-background scroll-mt-24">
        <div className="container mx-auto px-6 lg:px-10 py-20 md:py-28">
          <div className="grid md:grid-cols-2 gap-12 md:gap-20">
            <div>
              <p className="text-[11px] uppercase tracking-[0.28em] text-background/60 mb-4">Talk to us</p>
              <h2 className="font-display font-light text-3xl md:text-5xl tracking-[-0.02em] leading-[1.05]">
                {division.ctaLabel}.
              </h2>
              <p className="mt-8 text-background/70 leading-relaxed max-w-md">
                Tell us about the site and what is not working. {BRAND.ownerReplyLine}
              </p>
            </div>
            <EnquiryForm defaultService={division.leadService} />
          </div>
        </div>
      </section>

      <Footer />
      <StickyContactBar />
    </div>
  );
};

export default ServiceDivisionPage;
