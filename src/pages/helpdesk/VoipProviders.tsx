...
const VoipProviders = () => {
  ...
  const [form, setForm] = useState({ name: "", contact_person: "", email: "", phone: "", website: "", services: "", account_manager: "", notes: "", is_active: true });

  const { data: providers = [], isLoading } = useQuery({
    queryKey: ["voip-providers"],
    queryFn: async () => {
      const { data, error } = await supabase.from("voip_providers").select("*").order("name");
  ...
  const upsert = useMutation({
    mutationFn: async () => {
      if (editId) {
        const { error } = await supabase.from("voip_providers").update({ ...form, updated_at: new Date().toISOString() }).eq("id", editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("voip_providers").insert(form);
        if (error) throw error;
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["voip-providers"] }); toast.success(editId ? "Updated" : "Added"); setOpen(false); resetForm(); },
    onError: (e: any) => toast.error(e.message),
  });

  const remove = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("voip_providers").delete().eq("id", id); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["voip-providers"] }); toast.success("Deleted"); },
  });

  const resetForm = () => { setEditId(null); setForm({ name: "", contact_person: "", email: "", phone: "", website: "", services: "", account_manager: "", notes: "", is_active: true }); };

  const openEdit = (p: any) => {
    setEditId(p.id);
    setForm({ name: p.name, contact_person: p.contact_person || "", email: p.email || "", phone: p.phone || "", website: p.website || "", services: p.services || "", account_manager: p.account_manager || "", notes: p.notes || "", is_active: p.is_active });
    setOpen(true);
  };
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-2xl font-bold text-foreground">VoIP Providers</h1>
          <div className="flex gap-3">
            ...
            <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) resetForm(); }}>
              ...
              <DialogContent className="max-w-lg">
                <DialogHeader><DialogTitle>{editId ? "Edit" : "New"} VoIP Provider</DialogTitle></DialogHeader>
              ...
              <TableBody>
                {filtered.map((p: any) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="max-w-[150px] truncate">{p.services || "—"}</TableCell>
                ...
                {filtered.length === 0 && <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">No VoIP providers found.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </div>
        )}
