-- Fix policies creation without IF NOT EXISTS

-- blog_views table + indexes already may exist; ensure RLS and policies
...
-- policies
drop policy if exists "blog_views_select_all" on public.blog_views;
create policy "blog_views_select_all" on public.blog_views for select using (true);

drop policy if exists "blog_views_insert_all" on public.blog_views;
create policy "blog_views_insert_all" on public.blog_views for insert with check (true);

drop policy if exists "blog_views_update_all" on public.blog_views;
create policy "blog_views_update_all" on public.blog_views for update using (true) with check (true);
...
create trigger blog_views_set_last before update on public.blog_views for each row execute procedure public.set_last_view_at();
...
-- policies for notes
drop policy if exists "blog_notes_select_approved" on public.blog_notes;
create policy "blog_notes_select_approved" on public.blog_notes for select using (approved = true);

drop policy if exists "blog_notes_insert_all" on public.blog_notes;
create policy "blog_notes_insert_all" on public.blog_notes for insert with check (true);
