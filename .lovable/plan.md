...
- On-device signing: `LoggedCallView` gains a kiosk-style "Hand over to client to sign" action that renders the Satio sheet itself in a full-screen overlay (admin navigation hidden), with the sheet's sign-off block turned into live inputs: first name, surname, `SignaturePad` canvas, signed date + time (prefilled from `now()`, editable), rating and comment. Submits through the same public `job-card-signoff` sign action and token; existing one-time lock and validation unchanged. Split name is stored into the existing `signed_by_name` field (joined) so no schema change is needed; touch targets min 44px.
